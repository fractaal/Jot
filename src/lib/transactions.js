...
function normalizeTransactionInput(payload, { timezone = 'Asia/Manila', categories = [] } = {}) {
  const dayjs = ensureDayJs();
  ...
  const rawInput = normalizeString(payload.rawInput || payload.raw_input || payload.originalText || payload.original_text);
